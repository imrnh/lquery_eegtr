...
def train_one_step(batch, model, spectral_conv, optimizer, scaler, kl_loss, device):
    ...
    with autocast("cuda", dtype=torch.float16):
        Xf, Xt = spectral_conv(spectrograms)
        out = model(Xt, Xf)
...
def evaluate(model, spectral_conv, val_dataloader, kl_loss, device):
    ...
    with torch.no_grad():
        for val_batch in val_dataloader:
            ...
            with autocast(device_type="cuda", dtype=torch.float16):
                Xf_val, Xt_val = spectral_conv(val_spectrograms)
                val_out = model(Xt_val, Xf_val)
...
def train(
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        for batch in t:
            ...
            # Training step
            loss = train_one_step(batch, model, spectral_conv, optimizer, scaler, kl_loss, device)
            epoch_loss += loss
            t.set_postfix({'KL_loss': loss})
            
            # Save checkpoint periodically
            if config.step % config.training_state_checkpoint_frequency == 0:
                save_checkpoint(checkpoint_path, epoch, config.step, model, spectral_conv, optimizer, scaler)
                print(f"\n[Step {config.step}] Checkpoint saved to {checkpoint_path}")
            
            # Evaluation
            if config.step % config.eval_interval == 0:
                val_loss = evaluate(model, spectral_conv, val_dataloader, kl_loss, device)
                ...
                save_checkpoint(save_path, epoch, config.step, model, spectral_conv, optimizer, scaler, val_loss)
